...
def chat(session_id=None):
    ...
    subjects = DepartmentService.get_subjects_for_class(current_user.class_id) if current_user.class_id else []
...
def _load_user_for_ai():
    ...
    _ = user.student_class.name if user.student_class else None
    return user
...
def subjects():
    subjs = DepartmentService.get_subjects_for_class(current_user.class_id) if current_user.class_id else []
...
def assignments():
    pagination = DownloadService.get_all(
        page=request.args.get('page', 1, type=int),
        category='assignment', department_id=current_user.department_id,
    )
    return render_template('student/assignments/index.html', pagination=pagination)
